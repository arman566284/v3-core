...
contract TestNevorV3Router is INevorV3SwapCallback {
    ...
    function nevorV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? INevorV3Pool(msg.sender).token0() : INevorV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == INevorV3Pool(pools[0]).token1();
            INevorV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(INevorV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(INevorV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
