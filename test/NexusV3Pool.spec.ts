...
import { TestNexusV3SwapPay } from '../typechain/TestNexusV3SwapPay'
...
import { TestNexusV3ReentrantCallee } from '../typechain/TestNexusV3ReentrantCallee'
...
describe('NexusV3Pool', () => {
  ...
  let factory: NexusV3Factory
  let pool: MockTimeNexusV3Pool

  let swapTarget: TestNexusV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeNexusV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#feeProtocol', () => {
    ...
    describe('#collectProtocol', () => {
      ...
      it('respects a custom feeRecipient set on the factory', async () => {
        await pool.setFeeProtocol(6, 6)

        // Set custom fee recipient on the factory
        const customRecipient = '0x0000000000000000000000000000000000001337'
        await factory.setFeeRecipient(customRecipient)
        expect(await factory.feeRecipient()).to.be.eq(customRecipient)
        ...
        // Even though collectProtocol is called with other.address, it transfers to customRecipient
        await expect(pool.collectProtocol(other.address, MaxUint128, MaxUint128))
          .to.emit(token0, 'Transfer')
          .withArgs(pool.address, customRecipient, '83333333333332')

        // Clean up: reset fee recipient back to address(0)
        await factory.setFeeRecipient(ethers.constants.AddressZero)
        expect(await factory.feeRecipient()).to.be.eq(ethers.constants.AddressZero)
      })

      it('only factory owner can set custom feeRecipient', async () => {
        const customRecipient = '0x0000000000000000000000000000000000001337'
        await expect(factory.connect(other).setFeeRecipient(customRecipient)).to.be.reverted
      })
    })
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestNexusV3ReentrantCallee')
      ).deploy()) as TestNexusV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestNexusV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestNexusV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestNexusV3SwapPay
