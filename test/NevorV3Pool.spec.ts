...
import { TestNevorV3SwapPay } from '../typechain/TestNevorV3SwapPay'
...
import { TestNevorV3ReentrantCallee } from '../typechain/TestNevorV3ReentrantCallee'
...
describe('NevorV3Pool', () => {
  ...
  let factory: NevorV3Factory
  let pool: MockTimeNevorV3Pool

  let swapTarget: TestNevorV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeNevorV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestNevorV3ReentrantCallee')
      ).deploy()) as TestNevorV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestNevorV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestNevorV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestNevorV3SwapPay
