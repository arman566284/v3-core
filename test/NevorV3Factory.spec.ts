...
describe('NevorV3Factory', () => {
  ...
  const fixture = async () => {
    ...
    return (await factoryFactory.deploy()) as NevorV3Factory
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('NevorV3Pool')).bytecode
  })
  ...
  it('initial enabled fee amounts', async () => {
    expect(await factory.feeAmountTickSpacing(FeeAmount.CUSTOM)).to.eq(TICK_SPACINGS[FeeAmount.CUSTOM])
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('NevorV3Pool')
